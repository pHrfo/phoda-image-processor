	...
	for (var i = 0; i< h; i++){
		for (var j = 0; j < w; j++){
			...
			for(var i = 0; i < fKernel.length; i++){
				means.r += img.data[fKernel[i]]
				means.g += img.data[fKernel[i] + 1]
				means.b += img.data[fKernel[i] + 2]
			}
			var kCenter = (i * w + j) * 4
...
var harmonicMean = function(img,kerneloffset,m,n){
	...
	for (var i = 0; i< h; i++){
		for (var j = 0; j < w; j++){
			...
			for(var i = 0; i < fKernel.length; i++){
				means.r += 1/img.data[fKernel[i]]
				means.g += 1/img.data[fKernel[i] + 1]
				means.b += 1/img.data[fKernel[i] + 2]
			}
			var kCenter = (i * w + j) * 4
...
var charmonicMean = function(img,kerneloffset,q){
	...
	for (var i = 0; i< h; i++){
		for (var j = 0; j < w; j++){
			...
			for(var i = 0; i < fKernel.length; i++){
				means_q.r += Math.pow(img.data[fKernel[i]],q)
				means_q.g += Math.pow(img.data[fKernel[i] + 1],q)
				means_q.b += Math.pow(img.data[fKernel[i] + 2],q)

				means_q1.r += Math.pow(img.data[fKernel[i]],q+1)
				means_q1.g += Math.pow(img.data[fKernel[i] + 1],q+1)
				means_q1.b += Math.pow(img.data[fKernel[i] + 2],q+1)
			}
...
var Max = function(img,kerneloffset,m,n){
	...
	for (var i = 0; i< h; i++){
		for (var j = 0; j < w; j++){
			...
			for(var i = 0; i < fKernel.length; i++){
				if(means.r < img.data[fKernel[i]])
					means.r = img.data[fKernel[i]]
				
				if(means.g < img.data[fKernel[i] + 1])
					means.g = img.data[fKernel[i] + 1]
				
				if(means.b < img.data[fKernel[i] + 2])
					means.b = img.data[fKernel[i] + 2]
...
var midPoint = function(img,kerneloffset,m,n){
	...
	for (var i = 0; i< h; i++){
		for (var j = 0; j < w; j++){
			...
			for(var i = 0; i < fKernel.length; i++){
				if(min.r > img.data[fKernel[i]])
					min.r = img.data[fKernel[i]]
				
				if(min.g > img.data[fKernel[i] + 1])
					min.g = img.data[fKernel[i] + 1]
				
				if(min.b > img.data[fKernel[i] + 2])
					min.b = img.data[fKernel[i] + 2]

				if(max.r < img.data[fKernel[i]])
					max.r = img.data[fKernel[i]]
				
				if(max.g < img.data[fKernel[i] + 1])
					max.g = img.data[fKernel[i] + 1]
				
				if(max.b < img.data[fKernel[i] + 2])
					max.b = img.data[fKernel[i] + 2]
...
var meanFilters = function(){
	...
	var kerneloffset = Math.floor(Math.max(m,n)/ 2); // ????
	...
	switch(meanop) {
	    case 0:
	        img = arithmeticMean(img,kerneloffset,m,n);
	        break;
	    case 1:
	       	img = harmonicMean(img,kerneloffset,m,n);
	        break;
	    case 2:
	       	img = geometricMean(img,kerneloffset,m,n);
	        break;
	    case 3:
	        ...
	        break;
	    case 4:
	       	img = Max(img,kerneloffset,m,n);
	        break;
	    case 5:
	       	img = Min(img,kerneloffset,m,n);
	        break;
	    case 6:
	       	img = midPoint(img,kerneloffset,m,n);
	        break;
	    case 7:
	    	var d = document.getElementById("d_value").value;
	       	img = alphaTrimmedMean(img,kerneloffset,m,n,d);
	        break;
	    default:
	        ;
	}
